...
for (name in names(datasets)) {
  ...
  if (dataset$include){
    ...
    failuresPath <- file.path(outputFolder, "failures_Failures.txt")
    if (file.exists(failuresPath)) {
      saveExonFailures(failuresPath, bedFile, bamsDir, outputFolder)
    } else {
      message("No failures file produced — skipping saveExonFailures()")
      # optionally, still create an empty failedROIs.csv so downstream steps don’t break
      write.table(
        data.frame(SampleID=character(),
                   Chr=character(),
                   Start=integer(),
                   End=integer(),
                   Gene=character()),
        file.path(outputFolder, "failedROIs.csv"),
