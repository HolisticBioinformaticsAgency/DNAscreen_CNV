...
run_dnascreen() {
  ...
  samples_not_outlier_no_calls="${sim_dir}/${sim_type}/run${dnascreen_run}${work_folder_name}/samples_not_outlier_no_calls.txt"

  # Building vcf_lst.txt
  ls "${vcfs_and_bams_dir}/vcfs_run$dnascreen_run"/*.sorted.vcf.gz | xargs -n 1 basename > "run${dnascreen_run}${work_folder_name}/vcf_lst.txt"
