...
while IFS=$'\t' read -r full_sample_id chrom start end gene; do
    ...
    # Perform downsampling by excluding the specified region
    output_bam_file="$output_dir/cn${cnstate}_${full_sample_id}.bam"
    bedtools intersect -abam "$sample_bam_file" -b <(echo "$window") -v > "$output_bam_file"
