...
vcf_list="${sim_dir}/multiexon_sims/run${dnascreen_run}/vcf_lst.txt"
...
# # If there are fewer than 626 valid VCF files, display a message and exit
# if [ "$num_vcf_files" -lt 626 ]; then
#     echo "Error: Less than 626 VCF files available after filtering."
#     exit 1
# fi

# Randomly select 626 VCF files from the filtered list
selected_vcf_files=()
for ((i=0; i<626; i++)); do
...
echo "Randomly selected 626 VCF files that do not have sample IDs listed in $samples_with_cnvcalls."
