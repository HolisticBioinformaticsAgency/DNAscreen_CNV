...
SummaryStats <- setRefClass("SummaryStats",
                            ...
                            methods = list(
                              ...
                              getSamplesWithResults = function(data, bedFile){
                                ...
                                bedData <- read.csv(bedFile, header = F, sep = "\t", stringsAsFactors=F)
                              ...
                              fillGlobalStats = function(datasetName, alg){
                                ...
                                rownames(df) <- c("roi","roi_incNoCall","gene") # stats level
                                for (i in 1:nrow(df))
                                  df[i,] <- rep(0, 15)
                                ...
                                for(s in samples) {
                                  ...
                                  df["roi", "total"] <- df["roi", "total"] + s$algorithms[[alg]]$nROIs - s$algorithms[[alg]]$nFailedROIs
                                  ...
                                  df["roi_incNoCall", "total"] <- df["roi_incNoCall", "total"] + s$algorithms[[alg]]$nROIs
                                  # df["roi_incNoCall", "no_calls"] <- df["roi_incNoCall", "no_calls"] + s$algorithms[[alg]]$nFailedROIs
                                  df["roi_incNoCall", "no_calls"] <- NA
                                  ...
                                  # df["whole_strategy", "no_calls"] <- NA
                                  df["gene", "TP"] <- df["gene", "TP"] + s$algorithms[[alg]]$nTPgene_incNoCall
                                  df["gene", "TN"] <- df["gene", "TN"] + s$algorithms[[alg]]$nTNgene_incNoCall
                                  df["gene", "FP"] <- df["gene", "FP"] + s$algorithms[[alg]]$nFPgene_incNoCall
                                  df["gene", "FN"] <- df["gene", "FN"] + s$algorithms[[alg]]$nFNgene_incNoCall
                                  df["gene", "total"] <- df["gene", "total"] + s$algorithms[[alg]]$nCheckedGenes
                                  df["gene", "no_calls"] <- NA
                                }
...
SampleStats <- setRefClass("SampleStats",
                           ...
                           methods = list(
                             ...
                             # Check found positives against validated positives. Sets stats vars
                             loadResults = function(algorithm, foundPositives, failedROIs) {
                               ...
                               # Calculate ROIs stats excluding no-calls
                               for (i in 1:nrow(validatedROIs)) {
                                 row <- validatedROIs[i, ]
                                 isFailedROI <- (nrow(subset(sas$failedROIs, Start == row$start)) > 0)
                                 eval <- row$eval
                                 
                                 if (isFailedROI == FALSE) {
                                   # Normal regions
                                   if (eval == "tp") {
                                     sas$nTP <- sas$nTP + 1
                                   } else if (eval == "fn") {
                               ...
                               for (g in validatedGenes){
                                 ...
                                 if ("tp" %in% geneValidatedROIs$eval){  # TP if at least one roi was found by alg
                                   ...
                                   sas$FNdata[sas$nFNgene_incNoCall, ] <- list(g, type)
                               ...
                               # # Calculate stats at gene level and excluding no-calls
                               # for (g in validatedGenes){
                               #   geneValidatedROIs <- subset(validatedROIs, gene == g)
...
SampleAlgStats <- setRefClass("SampleAlgStats",
                              ...
                              methods = list(
                                ...
                                new = function(foundPositives, nROIs, fROIs){
                                  ...
                                  # should i not define "FailedGenes" as genes where all ROIs in the gene fail?
                                ...
                                countCheckedGenes = function(){
                                  nCheckedGenes <<- nTPgene_incNoCall + nTNgene_incNoCall + nFPgene_incNoCall + nFNgene_incNoCall
                                }
                              ))
