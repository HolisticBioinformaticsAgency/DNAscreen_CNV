...
output_dir="${sim_dir}/clinvar_sims/run${dnascreen_run}/output_cn${cnstate}"
selected_regions="${sim_dir}/clinvar_sims/run${dnascreen_run}/selected_regions/selected_regions_cn${cnstate}_sample"
...
while IFS=$'\t' read -r sample_id chrom start end gene; do
    ...
    # Window of CNV for downsampling
    ...
    bedtools subtract -abam "$sample_bam_file" -b <(echo "$window") > "$output_dir/original_$sample_id.bam"
    ...
    picard MergeSamFiles INPUT="$output_dir/downsampled_$sample_id.bam" INPUT="$output_dir/extracted_$sample_id.bam" OUTPUT="$output_dir/cn${cnstate}_$sample_id.bam"
