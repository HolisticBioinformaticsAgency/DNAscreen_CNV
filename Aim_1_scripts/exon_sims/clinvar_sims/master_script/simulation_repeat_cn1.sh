...
while IFS=$'\t' read -r full_sample_id chrom start end gene; do
    ...
    samtools view -s 0.5 -b "$output_dir/extracted_$sample_id.bam" > "$output_dir/downsampled_$sample_id.bam"
    samtools index "$output_dir/downsampled_$sample_id.bam"

    picard MergeSamFiles INPUT="$output_dir/downsampled_$sample_id.bam" INPUT="$output_dir/original_$sample_id.bam" OUTPUT="$output_dir/cn${cnstate}_$full_sample_id.bam"
    samtools index "$output_dir/cn${cnstate}_$full_sample_id.bam"

    rm -f "$output_dir/extracted_$sample_id.bam" "$output_dir/extracted_$sample_id.bam.bai"
    rm -f "$output_dir/original_$sample_id.bam" "$output_dir/original_$sample_id.bam.bai"
    rm -f "$output_dir/downsampled_$sample_id.bam" "$output_dir/downsampled_$sample_id.bam.bai"
