...
while IFS=$'\t' read -r sample_id chrom start end gene; do
    ...
    echo "Simulated CNV window: $window""${chrom}	${start}	${end}"
    ...
    # Downsample original.bam (P=0.5 retaining probability)
    samtools view -s 0.5 -b "$output_dir/original_$sample_id.bam" > "$output_dir/downsampled_$sample_id.bam"
